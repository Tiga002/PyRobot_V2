...
def get_repr_from_image(images_reshaped, modalities, data_augment, encoder,
    ...
    with slim.arg_scope(resnet_v2.resnet_utils.resnet_arg_scope(
        ...
        x, end_points = \
            resnet_v2_50(x, num_classes=None, global_pool=False, output_stride=None,
                         reuse=None, scope=scope_name, is_training=resnet_is_training)
    ...
    with tf.name_scope('rotate'):
        ...
        if type(preds) != list:
            rotated_preds, valid_mask_warps = \
                dense_resample(preds, flow_op, output_valid_mask)
        else:
            rotated_preds = []
            valid_mask_warps = []
            for pred in preds:
                rotated_pred, valid_mask_warp = \
                    dense_resample(pred, flow_op, output_valid_mask)
        ...
        with slim.arg_scope([slim.batch_norm], updates_collections=updates_collections) as arg_sc:
            ...
            for i in range(len(arg_sc.keys())):
                ...
                if 'convolution' in list(arg_sc.keys())[i]:
                    if 'normalizer_params' in list(arg_sc.values())[i]:
            ...
            with slim.arg_scope(arg_sc):
                ...
                x, outs = resnet_v2.resnet_v2(x, blocks, num_classes=None, global_pool=False,
                                              ...
                                              reuse=False, scope=name, is_training=is_training)
    return x, outs
...
def running_combine(fss_logits, confs_probs, incremental_locs,
    ...
    with tf.name_scope('combine_{:d}'.format(num_steps)):
        ...
        incremental_thetas_ = tf.unstack(
            incremental_thetas, axis=1, num=num_steps)
        ...
        for i in range(num_steps):
            ...
            running_sum_num = running_sum_num + \
                              fss_logits_[i] * confs_probs_[i]
...
def get_map_from_images(imgs, mapper_arch, task_params, freeze_conv, wt_decay,
    ...
    n_views = 1
    MapperTensors = recordclass('MapperTensors',
    ...
    x, out.vars_to_restore = get_repr_from_image(
        images_reshaped, ['rgb'], task_params.data_augment,
    ...
    if mapper_arch.dim_reduce_neurons > 0:
        ...
        batch_norm_param['activation_fn'] = \
            getattr(tf.nn, batch_norm_param['activation_fn'])
    ...
    with tf.variable_scope('fc'):
        ...
        f, _ = fc_network(f, neurons=neurons, wt_decay=wt_decay,
                          name='fc', offset=0,
