...
class Robot:
    ...
    def __init__(self,
        ...
        """
        root_path = os.path.dirname(os.path.realpath(__file__))  # path of this file as root path
        ...
        for srobot in robot_pool:
            if srobot in robot_name:
                ...
                mod = importlib.import_module(root_node + 'cfg.' +
                                              '{:s}_config'.format(srobot))  #importing pyrobot.cfg.locobot_config
                cfg_func = getattr(mod, 'get_cfg')  # cfg_func = get_cfg(base_type='kobuki') from pyrobot.cfg.locobot_config
                if srobot == 'locobot' and 'lite' in robot_name:
                    self.configs = cfg_func('create')
                else:
                    self.configs = cfg_func()  # this will load a set of configurations from cfg/config.py+cfg/locobot_config.py
        ...
        root_node += '.'  # root_node = pyrobot.locobot.
        ...
        if self.configs.HAS_BASE and use_base:
            mod = importlib.import_module(root_node + 'base')          # import pyrobot.locobot.base
            base_class = getattr(mod, self.configs.BASE.CLASS)         # base_class = 'LoCoBotBase' && setting up the pathplanner and low level controller
            self.base = base_class(self.configs, **base_config)        # load the configs of base from self.config to create the base_class || **base_config=base_config{}
        if self.configs.HAS_CAMERA and use_camera:
            mod = importlib.import_module(root_node + 'camera')        # import pyrobot.locobot.camera
            camera_class = getattr(mod, self.configs.CAMERA.CLASS)     # camera_class = 'LoCoBotCamera'
            self.camera = camera_class(self.configs, **camera_config)  # load the configs of camera from self.config to create the camera_class
...
class Base(object):
    ...
    specific Base classes would be built. (Parent Class of LoCoBotBase)
    """
