...
class XYTState(object):
    ...
    def update(self, x, y, theta):
        ...
        theta = wrap_theta(theta - self.old_theta) + self.old_theta  #wrap theta inbetween 0-2pi
...
class BaseState(BaseSafetyCallbacks):
    def __init__(self, base, build_map, map_img_dir, configs):
        ...
        s = rospy.Subscriber(configs.BASE.ROSTOPIC_ODOMETRY, Odometry,
                             lambda msg: self._odometry_callback(msg, 'state'))  #/odom
        self.subscribers = [s]

        BaseSafetyCallbacks.__init__(self, base)  #base means kibuki / create
...
class LoCoBotBase(Base): # LoCoBotBase is inherit from pyrobot.core.base
    """
    ...
    def __init__(self,
        ...
        self.base_state = BaseState(base, self.build_map, map_img_dir, configs)  #base_state will continuously subscribing /odom in behind

        # Path planner
        if base_planner is None:
            base_planner = configs.BASE.BASE_PLANNER
        assert (base_planner in ['movebase', 'none']), \
            'BASE.[BASE_PLANNER] should be movebase or none.'  # path planning is done by movebase [ROS], probably DWA planner
    ...
    def go_to_relative(
        ...
        # convert the relative position to absolute position ref. to world frame
    ...
    def go_to_absolute(
        ...
        try:
            #if we are using a map, we extract the path planned by the Global Planner
            if use_map:
                assert (
                    self.build_map), 'Error: Cannot use map without ' \
                                     'enabling build map feature'
                if self.base_controller == 'ilqr':
                    #goto = self.go_to_relative(close_loop=close_loop, smooth=smooth)
                    # while close_loop && smooth = true
                    goto = partial(
                        self.go_to_relative,
                        close_loop=close_loop,
                        smooth=smooth)
                    # goto function moves the robot the appropiate relative goal
                    # while NOT taking map into account.
                    # goto function pass to planner.move_to_goal
                    # as there is no go_to_relative() in Class MoveBasePlanner
                    ...
                    return
            # No matter using a map or not
            # the actual function driving the robot to the goal is
            # [ controller.go_to_absolute() ]
