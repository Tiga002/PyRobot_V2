...
class MoveBasePlanner:
    ...
    def __init__(self, configs):
        ...
        # wait until '/move_base/GlobalPlanner/make_plan' service is available
        rospy.wait_for_service(self.configs.BASE.PLAN_TOPIC, timeout=3)
        try:
           # Create a callable service named '/move_base/GlobalPlanner/make_plan',
           # with a serve type GetPlan from nav_msgs 
        ...
        # build start_state as a PoseStamped msg
    ...
    def _compute_relative_ang_dist(self, point2):
        ...
        # Transform point 2 into robot_base frame
        ...
        # Return angle and distance of point 2 from base_frame
        return (angle, distance)
    # Get the global plan from the planning service 
    def get_plan_absolute(self, x, y, theta):
        ...
        # Request a response from the planning_service by 
        # sending start,goal and tolerance to the server
    ...
    def move_to_goal(self, goal, go_to_relative):
        ...
        while g_distance > self.configs.BASE.TRESHOLD_LIN:  #0.15
        ...
        g_angle, g_distance = self._compute_relative_ang_dist(goal)
        # Adjust the orientation of the robot when it arrives the goal point
...
class TrajectoryTracker(object):
    ...
    def execute_plan(self, plan, close_loop=True):
        ...
        for i in range(plan.T):
            ...
            # Publish the Twist Msg Here ~!!!
            # ctrl_pub is from Class[LoCoBoTBase]::Class[Base]
